...
export default function register() {
  ...
  const registerHandler = async () => {
    ...
    setErrorMsg(false)
    setSuccess(false)
    Object.values(newUser).forEach(j => { if (j === "" || j === 0) return })
    if (newUser.age > 115) {
  ...
  return (
    <div id={styles.registerWrapper}>
      <section id={styles.registerSection}>
        ...
        <div className={styles.registerSectionRow} id={styles.registerButtonRow}>
          <button className={styles.registerButton} id={styles.registerButtonLogin} onClick={(e) => loginHandler(e.target.value)}>Login</button>
          <button className={styles.registerButton} id={styles.registerButtonRegister} onClick={(e) => registerHandler(e.target.value)}>Register</button>
        </div>
        <div className={styles.registerSectionErrorRow}>
          { errorMsg ? <span id={styles.registerErrorText}>{errorMsg}</span> : "" }
          { loading ? <span id={styles.registerLoadingText}>Loading...</span> : "" }
          { success ? <span id={styles.registerSuccessText}>Successfully created new user!</span> : ""}
        </div>
      </section>
    </div>
  );
